        ...
        Sub()
            If InnerState = MyLoadingState.Error Then
                If TextErrorInherit AndAlso State.IsLoader Then
                    ...
                    Else
                        Ex = Ex.RootException()
                        LabText.Text = If(Ex.IsBadNetwork(), "网络连接异常，请检查网络设置或代理配置。", Ex.Message)
                    End If
                Else
...
#Region "锟斤拷锟斤拷"
    ...
    Private ReadOnly LoopTimer As New System.Windows.Threading.DispatcherTimer With {
        .Interval = TimeSpan.FromMilliseconds(1800)
    }

    Private Sub ScheduleNextLoop()
        LoopTimer.Stop()
        RemoveHandler LoopTimer.Tick, AddressOf LoopTimer_Tick
        AddHandler LoopTimer.Tick, AddressOf LoopTimer_Tick
        LoopTimer.Start()
    End Sub

    Private Sub LoopTimer_Tick(sender As Object, e As EventArgs)
        LoopTimer.Stop()
        IsLooping = False
        AniLoop()
    End Sub

    Private Sub StopAnimationLoop()
        LoopTimer.Stop()
        IsLooping = False
    ...
    Private Sub AniLoop()
        ...
        If Not HasAnimation OrElse Not InnerState = MyLoadingState.Run OrElse AniSpeed > 10 OrElse Not IsLoaded Then
            StopAnimationLoop()
            Return
        End If
